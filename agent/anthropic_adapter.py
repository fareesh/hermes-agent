...
def build_anthropic_kwargs(
    ...
    #
    # Kimi's /coding and Moonshot endpoints speak the Anthropic Messages
    # protocol and support thinking via the standard ``thinking`` parameter.
    # ``copy_reasoning_content_for_api`` (run_agent.py) pads all assistant
    # tool-call messages with a space placeholder, and
    # ``convert_messages_to_anthropic`` (line 1687) converts it to a
    # ``thinking`` block — so the ``thinking.enabled`` request won't 400
    # on missing ``reasoning_content``.  Per-turn thinking budget is
    # controlled by ``reasoning_config`` (disabled for tool-call re-entries
    # to save quota, enabled for fresh user turns).
